...
- Auth: `POST /api/v1/auth/register` es auto-registro publico, siempre crea `role: "USER"`, devuelve `LoginResponse` (token + user) porque loguea a quien se registro; `POST /api/v1/auth/login` emite JWT por cedula y conserva fallback demo configurable por ambiente. El JWT lleva el `role` del usuario como claim (`JwtService`); `JwtAuthenticationFilter` lo usa para otorgar `ROLE_<role>`.
- Usuarios (admin): `POST /api/v1/users` (`UserController`/`UserService`) es un endpoint separado, exclusivo de `ADMIN` (`SecurityConfig.hasRole("ADMIN")`, mismo patron que `/api/v1/email-jobs/**`), para que un admin cree la cuenta de otra persona — `role` opcional en el body (default `"USER"`, tambien acepta `"ADMIN"` porque el caller ya es admin). No emite `LoginResponse`/token para la cuenta creada (`UserResponse`, solo datos basicos) — no es un login, es una accion administrativa. Usado por `credit-web` en `/users` para crear comerciales de prueba.
